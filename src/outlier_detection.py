...
"""
@description:
This python file implements the outlier detection approach (i.e., One Class SVM).
...
OUTPUT:
    1) outlier detection results on testing pool : "../experiment/outlier_detection/"
...
def split_block(data):
    ...
    for rep in repeat:
        ...
        block = block.sort_values(data.columns[-1],ascending = False)
        anomaly_num = int(rep * anomaly_ratio)
        normaly_num = rep - anomaly_num
        Anomaly_indexes = [-1 * x for x in np.ones(anomaly_num, np.int)]  # anomaly samples: the best n
        noAnomaly_indexes = np.ones(normaly_num, np.int) # normal samples: the rest
        indexes = np.append(Anomaly_indexes, noAnomaly_indexes)
...
def model_classification(train_set,test):
    test_set = test['feature']
    # test_set = [i.split('[')[-1].split(']')[0].split(',') for i in test_set]  BUG-1: test_set should be in the form of numeric [patch:75-79]
    test_set_num = []
    ...
    test['splitblock_OneClassSVM'] = oneclasssvm(train_set, test_set_num)
    # print(test[:10])
    return test
  

def oneclasssvm(train_set, test):
    train = train_set.loc[train_set['isAnomaly']== 1]

    train_x = train[train.columns[:-1]]
    train_y = train[train.columns[-1]]
    clf = OneClassSVM(nu=anomaly_ratio, kernel="linear")
...
def outlier_detection():
    ...
    resultfolds = '../experiment/outlier_detection/'
    ...
    for dataset in dataset_lists: # in each dataset
        ...
        for i in range(rounds): # in each round
            ...
            result_rf = model_classification(anomaly_train, test)
