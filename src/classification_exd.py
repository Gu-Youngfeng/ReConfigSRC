...
"""
@description:
This python file implements the classification_exd approach (i.e., Random Forest).
...
OUTPUT:
    1) classification_exd results on testing pool : "../experiment/classification_exd/"
...
def remark_validation(validation):
    # only use the actual performance in the validation pool
    validation = validation.sort_values(by=["act_performance"])
    validation_independent = validation[validation.columns[:-1]]
    result_isAnomaly = validation[validation.columns[:len(validation_independent.columns)]]

    len_validation = len(validation)
    labels = []
    for i in range(len_validation):
        if i < len_validation*CLASS_RATIO:
            labels.append(1)
        else:
            labels.append(-1)

    result_isAnomaly["isAnomaly"] = labels
    # result_isAnomaly.to_csv("validation01.csv", index=False)
    return result_isAnomaly
...
def rf_Anomaly_ratio(train,test): # here train refers to the prediction results from the validation set
    ...
    test_set_num = []
    # print("test features:", test["feature"][0])
...
def classification_exd():
    ...
    for dataset in dataset_lists[:]: # in each dataset
        if not os.path.exists(resultfolds + dataset):  # resultfolds + dataset = '../experiment/classification_exd/AJStats/'
        ...
        for i in range(rounds)[:]: # in each round
            ...
            anomaly_train = remark_validation(validation)
            ###################################################
